...
def load_dqn_model(model_path=f"./outputs/pacman_dqn_{constants.CURRENT_MAZE_TYPE.lower()}", strict=False):
    """
    Load pre-trained DQN model with improved error handling
    
    Parameters:
        model_path: Path to saved model
        strict: If True, raise error if model not found instead of returning None
        
    Returns:
        model: Loaded DQN model or None if loading fails (in non-strict mode)
    """
...
def evaluate_model(model, env, num_episodes=10):
    """
    Evaluate trained model over several episodes
