...
class PacmanEnv(gym.Env):
    ...
    def step(self, action):
        # Ensure action is a simple integer, not a numpy array
        action = int(action.item()) if hasattr(action, 'item') else int(action)
        
        # Apply action (0=right, 1=left, 2=down, 3=up)
        direction_map = {
        ...
        self.pacman.desired_direction = direction_map[action]
        self.pacman.last_action = action
        
        # Update ghost release timing (release a new ghost every 150 steps)
        if self.steps_taken % 150 == 0 and self.ghosts_to_release:
            self.active_ghosts.append(self.ghosts_to_release.pop(0))
        ...
        # Update game state
        self.pacman.update(self.maze, self.active_ghosts, self.pellet_grid)
        ...
        current_pos = (pacman_col, pacman_row)
        pellet_eaten = False
        ...
        if 0 <= pacman_row < constants.ROWS and 0 <= pacman_col < constants.COLS:
            if self.pellet_grid[pacman_row][pacman_col]:
                ...
                self.score += 10
                self.pellets_eaten += 1  # Track pellets eaten
                pellet_eaten = True
                self.last_pellet_steps = 0  # Reset counter when pellet eaten
                
        # Check for collision with ghosts
        done = False
        for ghost in self.active_ghosts:
            if check_collision_pacman_ghost(self.pacman, ghost):
                done = True
                break
        ...
        # End episode if stuck too long without eating pellets
        if self.last_pellet_steps >= self.stuck_threshold:
            done = True
        
        # Update position visit counts for better exploration metrics
        pos_tuple = (pacman_col, pacman_row)
        self.position_visit_counts[pos_tuple] += 1
        self.recent_positions.append(pos_tuple)
        
        # Calculate the nearest pellet for reward calculation
        nearest_pellet = None
        nearest_dist = float('inf')
        for r in range(constants.ROWS):
            for c in range(constants.COLS):
                if self.pellet_grid[r][c]:
                    dist = abs(pacman_col - c) + abs(pacman_row - r)
                    if dist < nearest_dist:
                        nearest_dist = dist
                        nearest_pellet = (c, r)
        
        # Check if Pacman moved at all
        pacman_moved = True
        if hasattr(self.pacman, 'last_positions') and len(self.pacman.last_positions) >= 2:
            if self.pacman.last_positions[-1] == self.pacman.last_positions[-2]:
                pacman_moved = False
        ...
        if done:
            if game_won:
                logging.info(f"Episode WON after {self.steps_taken} steps with score {self.score}")
            elif self.steps_taken >= self.max_steps:
                logging.info(f"Episode ended due to max steps ({self.max_steps}) with score {self.score}")
            elif self.last_pellet_steps >= self.stuck_threshold:
                logging.info(f"Episode ended due to being stuck for {self.last_pellet_steps} steps with score {self.score}")
            else:
                logging.info(f"Episode LOST (ghost collision) after {self.steps_taken} steps with score {self.score}")
        ...
        # Calculate revisit rate for info
        revisit_rate = self._calculate_revisit_rate()
        
        # Info dict with more detailed metrics
        info = {
            'score': self.score,
